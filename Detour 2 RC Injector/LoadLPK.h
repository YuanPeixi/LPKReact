...
extern "C" {
    ...
    typedef void (*EnableDescendant_t)(void);
    typedef void (*DisableDescendant_t)(void);
    typedef void (*SetFlag_t)(int);
...
EnableDescendant_t EnableDescendant = nullptr;
DisableDescendant_t DisableDescendant = nullptr;
SetFlag_t SetFlag = nullptr;
...
BOOL InitSimple(LPCSTR dllPath)
{
    ...
    EnableDescendant = (EnableDescendant_t)GetProcAddress(hDll, "EnableDescendant");
    DisableDescendant = (DisableDescendant_t)GetProcAddress(hDll, "DisableDescendant");

    SetFlag = (SetFlag_t)GetProcAddress(hDll, "SetFlag");

    // 检查是否所有必要函数都加载成功
    if (!FindInst || !CountInst || !InsertProt ||
        !FindProt || !RemoveProt || !CountProt ||
        !InstallGlobal || !RemoveGlobal || !RemoveGlobalImmediately||
        !EnableDescendant||!DisableDescendant||
        !SetFlag) {
        FreeLibrary(hDll);
        hDll = nullptr;
        return FALSE;
...
void CleanupSimple()
{
    ...
    EnableDescendant = nullptr;
    DisableDescendant = nullptr;
